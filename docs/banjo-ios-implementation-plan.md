...
| R10 | Legal/takedown posture shift upstream (RecompFrontend license unresolved; recompiled-code artifact) | Low | Distribution | Source-first; block IPA publication until the upstream license is authoritative; include exact corresponding source and notices; never bundle ROM; track upstream posture; feasibility L1-L3 |
...
7. **Upstream reception** of plume-iOS/UIKitWindow and the timer fix (DD6). Resolved for this build loop on 2026-07-29 by the read-only package in [`docs/UPSTREAM-HANDOFF.md`](UPSTREAM-HANDOFF.md), which defines the exact proposal boundaries without writing upstream. Reception remains intentionally unknown unless the project owner later authorizes contact.
