...
- **L2 — No LICENSE file** at BanjoRecomp root (RecompFrontend also lacks one; N64ModernRuntime and rt64 are MIT). Same gap HarkinianPad flagged for Shipwright. Mitigation: confirm intent with upstream before any binary redistribution; source-only until then.
